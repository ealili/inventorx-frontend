    ...
    if (user.id) {
      axiosClient.put(`password`, {
        ...
        .then(() => {
          ...
          setUser({...user, current_password: '', new_password: '', new_password_confirmation: ''})
          setDisplayBtn('')
  ...
  return (
    ...
    <div className={'card animated fadeInDown'}>
      ...
      <form style={{display: `${displayPassForm}`}} onSubmit={updatePassword}>
        {passwordErrors && <div className={'alert'}>
          {Object.keys(passwordErrors).map(key => (<p key={key}>{passwordErrors[key][0]}</p>))}
        </div>}

        <h2>Change your password</h2>
        <br/>
        <input onChange={e => setUser({...user, current_password: e.target.value})}
               placeholder={'Current Password'}
               type={'password'}/>
        <input onChange={e => setUser({...user, new_password: e.target.value})}
               placeholder={'New Password'}
               type={'password'}/>
        <input onChange={e => setUser({...user, new_password_confirmation: e.target.value})}
               placeholder={'New Password Confirmation'}
               type={'password'}/>
        <button type={'submit'} className="btn">Update Password</button>
        &nbsp;
        <button className={'btn'} type={'button'} onClick={handleCloseClick}>Close</button>

      </form>
      <button style={{display: `${displayBtn}`}} onClick={handleClick} className={'btn'}>Change
        Password
      </button>
    </div>
  </>);
